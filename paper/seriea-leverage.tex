...
costs $7.3$. The away fixture carries $2.25\times$ the leverage of the home one,
...
Results reproduce exactly on the machine that generated them---two independent
runs agree bit for bit---but not across machines. No random seed enters the
fit, so the only source of variation is floating-point reduction order; on
different hardware the converged parameters move in the fifth decimal, which
shifts two expected-calibration entries by $0.0003$ and flips one match of
$2{,}660$. Expect agreement to three decimal places rather than exactly.
