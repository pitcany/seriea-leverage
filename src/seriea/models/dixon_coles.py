...
#: Bounds on the low-score dependence parameter. Outside roughly this range the
#: tau correction can drive a score probability negative.
_RHO_BOUNDS: tuple[float, float] = (-0.15, 0.15)
...
class DixonColesForecaster:
    ...
    def _negative_log_likelihood(
        ...
        Returns:
            The weighted negative log likelihood, or a large finite penalty if
            the parameters make any observed scoreline impossible.
        """
    ...
    def _rates(self, fixtures: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
        ...
        def rating(names: pd.Series, table: np.ndarray) -> np.ndarray:
            positions = names.map(self._index)
            values = np.where(
                positions.isna(), 0.0, table[positions.fillna(0).to_numpy(dtype=int)]
            )
            return values.astype(float)

        home_attack = rating(fixtures["home"], self._attack)
        home_defence = rating(fixtures["home"], self._defence)
        away_attack = rating(fixtures["away"], self._attack)
        away_defence = rating(fixtures["away"], self._defence)
